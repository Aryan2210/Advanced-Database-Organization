...
extern RC createTable (char *name, Schema *schema)
{
	...
	initBufferPool(&recordManager->bufferPool, name, NUM_OF_MPAGES, RS_LRU, NULL);
...
extern RC openTable (RM_TableData *rel, char *name)
{
	...
	recordManager->numTuples= *(int*)pageHandle;
	pageHandle = pageHandle + sizeof(int);
...
{
	// Accessing our data structure's numTuples and returning it
	RecordManager *recordManager = rel->mgmtData;
	return recordManager->numTuples;
}
...
extern RC insertRecord (RM_TableData *rel, Record *record)
{
	...
	rId->page = recordManager->freePage;

	// Pinning page i.e. telling Buffer Manager that we are using this page
	pinPage(&recordManager->bufferPool, &recordManager->pageHandle, rId->page);
	...
	slotPointer = slotPointer + (rId->slot * recordSize);
...
extern RC startScan (RM_TableData *rel, RM_ScanHandle *scan, Expr *cond)
{
	...
	// Setting the tuple count
    	tableManager->numTuples = ATTR_SIZE;
...
extern RC next (RM_ScanHandle *scan, Record *record)
{
	...
	// Getting Scan Count
	int soc = scanManager->soc;

	// Getting tuples count of the table
	int numTuples = tableManager->numTuples;
	...
	{  
		...
		pinPage(&tableManager->bufferPool, &scanManager->pageHandle, scanManager->rId.page);
		...
		data = data + (scanManager->rId.slot * recordSize);
